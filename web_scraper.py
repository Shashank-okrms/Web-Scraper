...
phone_pattern = re.compile(r'(\+?\d{1,3}[\s.-]?)?\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}|\d{3}[\s.-]?\d{4}[\s.-]?\d{4}|(\d{2,4}\s\d{3,4}\s\d{3,4})')
...
def extract_info(url):
    try:
        ...
        if email_match:
            email = email_match.group(0).replace('email', '').strip()
        
        phone = None
        phone_match = phone_pattern.search(soup.text)
        if phone_match:
            phone = phone_match.group(0).replace('mobile', '').strip()
